...
atoms = [[(0, 0., 0.), 1., 1., 20]] # Atomic no is protons
...
xrange = np.linspace(-15., 15., fid)
yrange = np.linspace(-15., 15., fid)
zrange = np.linspace(-15., 15., fid)
...
@numba.njit
def elec_dens(fid, xrange, yrange, zrange, alpha, beta, evec, eval):
    ...
    def basis_func(i, alpha, beta, pos, atom_pos):
        ...
        li = abs(alpha * (beta ** (i)))
        return 0.5 * np.sqrt(1 / np.pi) * np.sqrt(4 / np.sqrt(np.pi)) * ((2 * li) ** (3 / 4)) * np.exp(-li * (r ** 2))

    dist_real = np.zeros((fid, fid, fid))
    ...
    for i in range(fid):
        for j in range(fid):
            for k in range(fid):
                ...
                for m in range(basis):
                    for n in range(basis):
                        f_m = basis_func(m, alpha, beta, [x, y, z], (0, 0, 0))
                        f_n = basis_func(n, alpha, beta, [x, y, z], (0, 0, 0))
                        c_m = evec.T[2, m]
                        c_n = evec.T[2, n]
                        dist_real[i, j, k] += c_m * c_n * f_m * f_n

    return np.asarray(dist_real)


dist_real = elec_dens(fid, xrange, yrange, zrange, alpha, beta, evec, eval)

''' Broadened Values '''
gamma_val = np.linspace(0.001, 0.8, 15)


@numba.njit
def elec_dens2(fid, xrange, yrange, zrange, alpha, beta, evec, eval, gamma_val):
    def lam(i, alpha, beta):
        return alpha * (beta ** (i))
        ...
        for i in range(fid):
            for j in range(fid):
                for k in range(fid):
                    ...
                    for m in range(basis):
                        for n in range(basis):
                            for p in range(int(len(evec.T) / 2)):
                                f_m = basis_func(m, alpha, beta, [x, y, z], (0, 0, 0))
                                f_n = basis_func(n, alpha, beta, [x, y, z], (0, 0, 0))
                                c_m = evec.T[p, m]
                                c_n = evec.T[p, n]
                                dist[i, j, k] += c_m * c_n * f_m * f_n * (
                                            (gamma) / ((elev - eval[p]) ** 2 + (0.5 * gamma) ** 2))
        broad_dist.append(dist)
    return broad_dist


broad_dist = np.asarray(elec_dens2(fid, xrange, yrange, zrange, alpha, beta, evec, eval, gamma_val))

''' Calculate the Sum of Squared Differences '''
ssd = []

dist_norm = dist_real / np.max(dist_real)

for g in range(len(gamma_val)):
    g_dist = broad_dist[g] / np.max(broad_dist[g])

    ssd_val = 0
    for i in range(fid):
        for j in range(fid):
            for k in range(fid):
                ssd_val += (dist_norm[i, j, k] - g_dist[i, j, k]) ** 2
    ssd.append(ssd_val)

fig, ax = plt.subplots()

ax.scatter(gamma_val, ssd)

ax.set_ylabel("Sum of Squared Differences")
ax.set_xlabel("Broadening ($\\gamma$)")
plt.show()