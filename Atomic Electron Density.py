...
def basis_func(i, alpha, beta, pos, atom_pos):
    ...
    theta = np.arctan2(np.sqrt((pos[0]-atom_pos[0])**2 + (pos[1]-atom_pos[1])**2),(pos[2]-atom_pos[2]))
...
def mol_density(fid, xrange, yrange, evec, atoms_new, alpha, beta):
            ...
            for k in range(fid):
                ...
                for m in range(len(evec)):
                    for n in range(len(evec)):
                        for a in atoms_new:
                            for p in range(0, 2):
                                if m in range(a[0][0], a[0][1] + 1):
                                    f_m = basis_func((m - a[0][0]), alpha, beta, [x, y, z], a[1])
                                    c_m = evec.T[p, m]
                                if n in range(a[0][0], a[0][1] + 1):
                                    f_n = basis_func((n - a[0][0]), alpha, beta, [x, y, z], a[1])
                                    c_n = evec.T[p, n]
...
dist, size_val = mol_density(fid, xrange, yrange, evec, atoms_new, alpha, beta)
...
''' 2D Electron Density Cross-section '''
fid = 60

xrange = np.linspace(-3., 3., fid)
yrange = np.linspace(-3., 3., fid)
...
dist = mol_density(fid, xrange, yrange, evec, atoms_new, alpha, beta)

''' 2D Electron density plot '''
fig, ax = plt.subplots()
...
ax.pcolormesh(xrange, yrange, dist.T, norm=norm, cmap=cmap)

ax.set_xlabel("x Position ($a_{0}$)")
ax.set_ylabel("y Position ($a_{0}$)")

fig.colorbar(sm, ax=ax, label="$|\\Psi|^{2} $")
plt.show()