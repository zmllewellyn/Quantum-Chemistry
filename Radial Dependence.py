...
def radial_energy(R):
    ...
    atoms = [[(-R / 2, 0., 0.), 1, 1., 20], [(R / 2, 0., 0.), 1, 1., 20]]  # Atomic no is protons # Atomic no is protons
...
def main():
    learner = adaptive.Learner1D(radial_energy, (0.1, 3.))
    learner.load(fname)
    ...
    runner = adaptive.Runner(
        ...
        goal=lambda l: l.npoints > 100000,
    )

    # periodically save the data (in case the job dies)
    runner.start_periodic_saving(dict(fname=fname), interval=300)
