...
''' Line Map of Lowest Energy State Value '''
Nstate = np.arange(1, 30)
...
for n in Nstate:
    ...
    # atoms = [[(0., 0., 0.), 1, 0.5, 20], [(0., 0., R), 1, 0.5, 20], [(0., 0., 0.7), 0, 0, 20], [(0., 0., -0.7), 0, 0, 20], [(0., 0., 2.1), 0, 0, 20]] # Atomic no is protons
    atoms = [[(0., 0., 0.), 1, 1, n]]  # Atomic no is protons
    ...
    e_ground.append(eval[0])
    # e_ground.append(E_new)

''' Plot line function '''
fig, ax = plt.subplots()

# axis, width, height
axins = ax.inset_axes([0.5, 0.5, 0.4, 0.4], xlim=(10,25), ylim=(-0.501, -0.498))

ax.scatter(Nstate, e_ground)
axins.scatter(Nstate[10:25], e_ground[10:25])

ax.set_xlabel("Number of Basis Functions")
ax.set_ylabel("Ground State Energy ($E_{h}$)")
